# clear workspace
rm(list = ls()) #this command clear what x and y stand for
 ...
 res # now this will becom exactly the same as what the file is
 ...
 attach(res) # make elements of res available in workspace; if we did not attach, we will call
 #every variable like res$TV
 TV # now works

 # use lm 
 
 fit1 = lm(sales ~ TV, data = res)
 fit1
 plot(sales ~ TV, data = res)
 plot(TV,sales) # plot Sales vs TV , X-axis is TV
 # plot(TV ~ sales, data = res)  x-axis is sales not TV, but should be TV, so 
 #not right
 ...
 # lines adds to existing plot, lwd: just draw the line on the graph
 ...
 plot(TV,sales, main= 'K = 300')
 lines(1:300,pr300, lwd=2, col = 2)
...
#----------------------------------------------------------------------
 ...
 lines(x, f ,col = 2, lwd=2) # add regression function to the plot
 ...
 # since if k <> n, the result of knn-reg will be different every time
...
m=1
m
for(i in 1:3){ # this is exactly three times
  m=m+1
}
m
