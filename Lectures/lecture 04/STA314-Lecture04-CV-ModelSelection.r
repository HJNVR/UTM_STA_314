...
# -----------------------------------------------------
# now use function above to do data analyis
# we use Credit data set

# model.matrix is useful for dealing with qualitative
...
cv_error = do_10cv_bestsubset(X,Y,dmax = 11)
which.min(colMeans(cv_error))

par(mfrow=c(1,2))
# cross-validation selects a model with 6 predictors
# take a look at the graph
plot(1:11, colMeans(cv_error),xlab = 'd', ylab = 'CV error')

# don't see too much, remove first couple of points
plot(3:11, colMeans(cv_error)[3:11],xlab = 'd', ylab = 'CV error')
...
plot(3:11, ylim = c(min(lb[3:11]),max(ub[3:11])), colMeans(cv_error)[3:11],xlab = 'd', ylab = 'CV error')
